...
public class HumanoidNetworkPlayer : NetworkBehaviour
{
    ...
    void Update()
    {
        ...
        //GRAB
        if (switchFP3P()) Debug.Log("Changed FP/3P state");
        ...
        if (avBody)
        {
            ...
            //2023-10-20: NEED TO FIX
            //So when you stand up and move then the hands move with you but the body not, so how do we capture the offset of the XR rig for
            //local movements vs movements with the the controller joysticks? (eg. if you stand up and take a step left/right or rotate)
            //=> HaCM transforms seem to behave differently than the myXRRig transform on local movements!! NEED TO TEST, maybe find alternative
            //for myXRRig.transform for the body
            //when fixed we can probably remove Init()
        }

        //GRAB
        if (!thirdPToggle.isOn) //only in First Person Mode 
        {
    ...
    bool switchFP3P() //for demo purposes, allows switching between first and 3rd person
    {
    ...
    { foreach (var x in transform.GetComponentsInChildren<SkinnedMeshRenderer>()) x.enabled = value; }
